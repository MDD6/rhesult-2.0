...
function buildPath(request: NextRequest) {
  ...
  return query ? `/api/entrevistas?${query}` : "/api/entrevistas";
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(buildEndpoint(apiBase, buildPath(request)), {
      method: "GET",
      cache: "no-store",
    });
...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(buildEndpoint(apiBase, "/api/entrevistas"), {
      method: "POST",
